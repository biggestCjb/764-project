...
class T5ContinualLearner:
    ...
    def unfreeze_task(self, task):
        ...
        # Define task similarity/difference
        similar_tasks = {
            'imdb': 'cb',
            'cb': 'imdb',
            'sst2': 'imdb',
        ...
        # Case 1: Unfreeze the most recent task
        if self.freeze_type == 'last':
            last_task_idx = self.task_list.index(task) - 1
            if last_task_idx >= 0:
                last_task = self.task_list[last_task_idx]
                print(f"Unfreezing the most recent task: {last_task}")
                self.freeze_unfreeze_mlps([last_task], requires_grad=True)

        # Case 2: Unfreeze the task most different from the current task
        elif self.freeze_type == 'different':
            different_tasks = different_tasks.get(task, None)
            if different_tasks:
                print(f"Unfreezing the task most similar to {task}: {different_tasks}")
                self.freeze_unfreeze_mlps([different_tasks], requires_grad=True)

        # Case 3: Unfreeze the task most similar to the current task
        elif self.freeze_type == 'similar':
            similar_task = similar_tasks.get(task, None)
            if similar_task:
                print(f"Unfreezing the task most similar to {task}: {similar_task}")
                self.freeze_unfreeze_mlps([similar_task], requires_grad=True)

        else:
            raise ValueError(f"Unknown freeze_type: {self.freeze_type}")
    ...
    def train_one_task(self,
        ...
        if self.prefix_MLPs!=None:
            ...
            self.unfreeze_task(task)  # 调用新的冻结逻辑，根据freeze_type解冻任务
